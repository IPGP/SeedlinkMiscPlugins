#TD_Modif, 02/2020
#In order to be able to automatically generate a unic proc/input_channel/channel_name mapping in plugins' setup.py, seedlink.source.type and seedlink.source.id are now set before calling pluginHandler.push().
#To generate a unic id at this step, we simply concatenate net+sta+source_alias.
#Moreover sproc_name setting is a little bit modified. If proc name starts with "auto" prefix, sproc_name is made unic by adding seedlink.source.id as suffix.
#These Changes are pointed out with the "#TD_modif" comment.
...
class Module(TemplateModule):
    ...
    def __process_station(self, profile):
        ...
        for source_type in self._get('sources').split(','):
            ...
            self._set('seedlink.source.type', source_type)#TD_modif
            self._set('seedlink.source.id', self.net+self.sta+source_alias)#TD_Modif
            ...
            if sproc_names:
                ...
                for sproc_name in sproc_names:
                    ...
                    if sproc:
                        if sproc_name.startswith("auto"):#TD_Modif
                            sproc_name=sproc_name+"_"+self._get('seedlink.source.id')#TD_Modif
